...
export async function POST(
  ...
  if (juz.status !== 'OPEN') return NextResponse.json({ error: 'Juz already claimed' }, { status: 409 });

  const updated = await prisma.khatmJuz.update({
    where: { id: juz.id },
    data: { status: 'READING', claimedByUserId: userId, claimedAt: new Date() },
    include: { claimedByUser: { select: { id: true, email: true, username: true, displayName: true } } },
  });

  return NextResponse.json({ juz: updated });
}
