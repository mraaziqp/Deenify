...
export function LearningAdminManager() {
  ...
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      try {
        const response = await fetch('/api/learning/questions?all=true');
        if (!response.ok) return;
        const data = await response.json();
  ...
  const handleSaveResource = async () => {
    ...
    try {
      ...
      const data = await response.json();
      const resource = data.resource as LearningResource;
        ...
        <TabsContent value="resources" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Search, edit, and publish learning materials.</CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Add or Update Resource</CardTitle>
              <CardDescription>Upload PDFs or cover images using Supabase Storage.</CardDescription>
            </CardHeader>
            ...
            <Card key={question.id}>
              <CardHeader>
                ...
                <CardDescription>
                  ...
                  {question.userName && <span className="ml-2">• Asked by {question.userName}</span>}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  {question.approvedAnswer || question.aiAnswer || 'Awaiting response.'}
                </p>
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Blur the field to approve and publish.</p>
                </div>
              </CardContent>
            </Card>
          ))}
